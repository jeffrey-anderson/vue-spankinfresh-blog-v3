  ...
  <div class="ml-3">
    <div v-if="article.id">
      <div class="form-group row">
        ...
        <div class="col-sm-10">
          <input type="text" readonly class="form-control-plaintext" id="articleIdField"
                 v-model="article.id"/>
        </div>
      </div>
      ...
      <div class="form-group row">
        ...
        <div class="col-sm-10">
          <div class="form-control-plaintext">{{ fancyDatePosted }}</div>
        </div>
      </div>
    ...
    <div class="form-group row">
      <label for="categoryField"
             v-bind:class="{'col-sm-2 col-form-label':true, 'text-danger' : categoryInvalid}">Category:</label>
      <div class="col-sm-10">
        <div v-if="article.id > 0">
          <input type="text" readonly class="form-control-plaintext" v-model="article.category"/>
        </div>
        <div v-else>
          <input type="text" v-bind:class="{'form-control':true, 'is-invalid' : categoryInvalid}"
                 id="categoryField" v-model="article.category"/>
        </div>
      </div>
    </div>
    ...
    <div class="form-group row">
      <label for="blogTitle"
             v-bind:class="{'col-sm-2 col-form-label':true, 'text-danger' : titleInvalid}">Title:</label>
      <div class="col-sm-10">
        <input type="text" v-bind:class="{'form-control':true, 'is-invalid' : titleInvalid}"
               id="blogTitle" v-model="article.title"/>
      </div>
    </div>
    ...
    <div class="form-group row">
      <label for="contentField"
             v-bind:class="{'col-sm-2 col-form-label':true, 'text-danger' : contentInvalid}">Content:</label>
      <div class="col-sm-10">
        <nav>
          <div class="nav nav-tabs" id="nav-tab" role="tablist">
            <a class="nav-item nav-link active" id="nav-markdown-tab" data-toggle="tab"
               href="#nav-markdown" role="tab" aria-controls="nav-markdown" aria-selected="true">Markdown</a>
            <a class="nav-item nav-link" id="nav-preview-tab" data-toggle="tab" href="#nav-preview"
               role="tab" aria-controls="nav-preview" aria-selected="false">Preview</a>
          </div>
        </nav>
        <div class="tab-content" id="nav-tabContent">
          <div class="tab-pane show active" id="nav-markdown" role="tabpanel"
               aria-labelledby="nav-markdown-tab">
            <textarea v-bind:class="{'form-control':true, 'is-invalid' : contentInvalid}"
                      id="contentField" rows="25" v-model="article.content"></textarea>
          </div>
