  ...
  <div>
    <div v-if="displayAll">
      ...
      <table class="table">
        ...
        <tr v-for="article in toc" :key="article.id">
          ...
          <td>{{ $filters.prettyDate(article.datePosted) }}</td>
          <td>
          ...
          <td class="row">
            <button type="button" class="btn btn-outline-dark mr-1" data-toggle="modal"
                    data-target="#exampleModal" @click="askToDelete(article)">
      ...
      <p class="lead text-center">
        <router-link class="btn btn-outline-primary btn-lg"
                     :to="{ name: 'edit-article', params: { id: 0 } }">Create a New Article
        </router-link>
      </p>
      <Modal
    ...
    <div v-else>
      ...
      <div v-for="(article, index) in toc" :key="article.id">
        <div v-if="index < 10">
          &#x25A0;
          <router-link :to="{ name: 'articles', params: { id: article.id } }">{{ article.title }}
          </router-link>
          <br/>
        </div>
      </div>
      <div v-if="toc.length > 10">
        &#x25A0; See
        <router-link :to="{ name: 'toc' }">all articles</router-link>
        <br/>
      </div>
    </div>
  </div>
